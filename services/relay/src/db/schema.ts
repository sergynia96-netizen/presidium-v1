...
export const users = pgTable(
  "users",
  {
    ...
    subscriptionTier: varchar("subscription_tier", { length: 20 })
      .default("free")
      .notNull(),
    privacyTier: varchar("privacy_tier", { length: 20 })
      .default("guardian")
      .notNull(),
    strikes: integer("strikes").default(0).notNull(),
    status: varchar("status", { length: 20 }).default("active").notNull(),
    presenceStatus: varchar("presence_status", { length: 20 }).default(
      "offline",
    ),
  ...
  (table) => [
    ...
    index("users_presence_lastseen_idx").on(
...
export const messages = pgTable(
  "messages",
  {
    ...
    senderDeviceId: uuid("sender_device_id").references(() => users.id, {
      onDelete: "set null",
    }),
  ...
  (table) => [
    ...
    index("messages_sender_device_idx").on(table.senderDeviceId),
    index("messages_direction_idx").on(table.direction),
    uniqueIndex("messages_sender_nonce_idx")
      .on(table.senderId, table.clientNonce)
...
export const stories = pgTable(
  "stories",
  {
    ...
    replyPermission: varchar("reply_permission", { length: 20 })
      .default("everyone")
      .notNull(),
...
export const marketplaceItems = pgTable(
  ...
  (table) => [
    ...
    index("marketplace_search_idx").on(
      table.category,
      table.status,
      table.createdAt,
    ),
